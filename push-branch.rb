...
unless ARGV.size >= 3 && ARGV[0].include?('/') && ARGV[1].include?(':')
  puts <<HELP
...
this script supports the following parameters:
  ...
  1 = login:auth token           (required; must contain : and the auth token must have write access to the repository)
...
github = Github.new(basic_auth: token)
begin
